...
void detectAndDisplayFaceAndEyes(cv::Mat frame)
{
	...
	for (size_t i = 0; i < faces.size(); i++)
	{
		...
		for (size_t j = 0; j < eyes.size(); j++)
		{
			...
			cv::Rect eyeRectR = getRightmostEye(eyes);
			cv::Mat eyeL = faceROI(eyeRectL); // crop the leftmost eye
			cv::Mat eyeR = faceROI(eyeRectR); // crop the rightmost eye
			cv::equalizeHist(eyeL, eyeL);
			cv::equalizeHist(eyeR, eyeR);
			std::vector<cv::Vec3f> circles;
			cv::HoughCircles(eyeL, circles, CV_HOUGH_GRADIENT, 1, eyeL.cols / 8, 250, 15, eyeL.rows / 8, eyeL.rows / 3);
			cv::HoughCircles(eyeR, circles, CV_HOUGH_GRADIENT, 1, eyeR.cols / 8, 250, 15, eyeR.rows / 8, eyeR.rows / 3);
			if (circles.size() > 0)
			{
				cv::Vec3f eyeballL = getEyeball(eyeL, circles);
				cv::Vec3f eyeballR = getEyeball(eyeR, circles);
				cv::Point centerL(eyeballL[0], eyeballL[1]);
				cv::Point centerR(eyeballR[0], eyeballR[1]);
				centers.push_back(centerL);
				centers.push_back(centerR);
				centerL = stabilize(centers, 5); // we are using the last 5
				centerR = stabilize(centers, 5); // we are using the last 5
				int radius = (int)std::round(circles[i][2]);
				cv::circle(frame, faces[0].tl() + eyeRectL.tl() + centerL, radius, cv::Scalar(0, 0, 255), 2);
				cv::circle(frame, faces[0].br() + eyeRectR.br() + centerR, radius, cv::Scalar(255, 0, 0), 2);
				cv::circle(eyeL, centerL, radius, cv::Scalar(255, 0, 0), 2);
				cv::circle(eyeR, centerR, radius, cv::Scalar(255, 255, 255), 2);
			}
			cv::imshow("Left Eye", eyeL);
			cv::imshow("Right Eye", eyeR);
		}
